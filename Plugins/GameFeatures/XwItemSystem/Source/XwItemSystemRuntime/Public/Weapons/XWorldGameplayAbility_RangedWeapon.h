...
#include "XWorldGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UXWorldRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class EXWorldAbilityTargetingSource : uint8
{
...
/**
 * UXWorldGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UXWorldGameplayAbility_RangedWeapon : public UXWorldGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UXWorldGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="XWorld|Ability")
	UXWorldRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UXWorldRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, EXWorldAbilityTargetingSource Source) const;
