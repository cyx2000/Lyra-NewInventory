...
#include "XWorldWeaponStateComponent.generated.h"
...
struct FXWorldScreenSpaceHitLocation
{
...
struct FXWorldServerSideHitMarkerBatch
{
	FXWorldServerSideHitMarkerBatch() { }

	FXWorldServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FXWorldScreenSpaceHitLocation> Markers;
...
UCLASS()
class UXWorldWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UXWorldWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FXWorldScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FXWorldScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FXWorldServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
