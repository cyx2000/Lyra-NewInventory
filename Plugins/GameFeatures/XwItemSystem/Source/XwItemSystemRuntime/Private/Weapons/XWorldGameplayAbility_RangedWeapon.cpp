...
#include "Weapons/XWorldGameplayAbility_RangedWeapon.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(XWorldGameplayAbility_RangedWeapon)

namespace XWorldConsoleVariables
{
...
UXWorldGameplayAbility_RangedWeapon::UXWorldGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UXWorldRangedWeaponInstance* UXWorldGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UXWorldRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UXWorldGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		if (GetWeaponInstance() == nullptr)
		{
			UE_LOG(LogLyraAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				...
				*UXWorldRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UXWorldGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UXWorldGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UXWorldGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Lyra_TraceChannel_Weapon;
}

FHitResult UXWorldGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UXWorldGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UXWorldGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, EXWorldAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UXWorldWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UXWorldWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != EXWorldAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == EXWorldAbilityTargetingSource::CameraTowardsFocus) || (Source == EXWorldAbilityTargetingSource::PawnTowardsFocus) || (Source == EXWorldAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == EXWorldAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == EXWorldAbilityTargetingSource::WeaponForward) || (Source == EXWorldAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == EXWorldAbilityTargetingSource::PawnTowardsFocus) || (Source == EXWorldAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UXWorldGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (XWorldConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, XWorldConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UXWorldGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());

	UXWorldRangedWeaponInstance* WeaponData = GetWeaponInstance();
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, EXWorldAbilityTargetingSource::CameraTowardsFocus);
		...
		if (XWorldConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, XWorldConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UXWorldGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UXWorldRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (XWorldConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, XWorldConsoleVariables::DrawBulletHitRadius, FColor::Red, false, XWorldConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UXWorldGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UXWorldGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UXWorldGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UXWorldWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UXWorldWeaponStateComponent>())
					{
...
void UXWorldGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UXWorldWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UXWorldWeaponStateComponent>();
